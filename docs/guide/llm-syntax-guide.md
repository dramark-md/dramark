...
2) 译配起始使用“= 空格原文”；单独“=”表示退出译配。
3) 唱段使用 $$ 包裹；唱段内可用 !!...!! 插入念白。
4) 行内 Tech Cue 必须是同一行内闭合的 <<...>>。
5) 代码块和行内代码中的特殊符号全部视为普通文本。
6) 若不确定，优先生成简单、可闭合、少嵌套结构。
```

## 十、最小自检清单（输出后）

1. 是否存在 `@角色 台词同一行`？有则改为两行。  
2. 是否把 `=原文` 写成了无空格形式？有则改为 `= 原文`。  
3. 每个 `$$`、`!!`、`<<<` 是否都能闭合？  
4. `<<...>>` 是否都在同一物理行闭合？  
5. 代码块/行内代码内是否误用了 DraMark 语义解释？
