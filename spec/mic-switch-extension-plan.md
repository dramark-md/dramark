# 麦克风分配/换麦语法扩展计划（草案，前端可配置解析）

> 状态：草案（frontend/app-layer），非 parser 核心语法落地项。
...
- 解析归属与边界：
  ...
  - app/扩展自行解析 `<<...>>` 内容识别换麦指令。
  - 2026-03-19 补充：micromark-integrated multipass parser 已保证 `<<...>>` 在常见 html-split 词法场景下仍归一化为 `inline-tech-cue`；应用层可优先消费该节点。
- 指令输入形态：
  - **行内 tech cue**：`<<...>>` 内使用简洁语法（`=`, `->`）。
