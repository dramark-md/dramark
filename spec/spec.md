...
- **技术标记 (Tech Cues)**：支持灯光、音响等技术提示的原生标记，助力剧本与舞台设计的无缝对接。
...
**解析边界**：parser 仅识别 `<<` / `>>` 外层边界并产出 Tech Cue 节点，`内容` 按原文透传为 payload，不在语法层进一步解析关键词或扩展指令。
...
**解析边界**：块级 Tech Cue 的原始 payload 必须保留；同时，payload 内属于 DraMark/CommonMark 的基础语法可按常规规则解析。关键词识别、换麦意图抽取等扩展语义由上层（frontend/app-layer）在消费节点时实现。
...
本节为应用层草案，不属于 parser 核心语法的强制落地范围。
由前端/扩展在消费 Tech Cue 节点时按需实现。

parser 负责产出 `inline-tech-cue` / `block-tech-cue` 节点并保留其原始 payload；对于 payload 内的基础 DraMark/CommonMark 语法可正常解析，但不负责把 payload 解释为换麦关键词、角色绑定或事件模型。
